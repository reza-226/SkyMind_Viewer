...
def to_numeric_ignore(series: pd.Series) -> pd.Series:
    """
    Emulate deprecated errors='ignore':
    - If the whole series can be converted, return numeric series.
    - If any value prevents conversion, return the original series unchanged.
    """
    try:
        return pd.to_numeric(series)  # default errors='raise'
    except Exception:
        return series
...
def parse_csv(file: io.BytesIO) -> pd.DataFrame:
    ...
    # Convert object-like numerics (preserve old ignore semantics)
    for c in list(df.columns):
        s = df[c]
        if s.dtype == object:
            try:
                s2 = s.astype(str).str.replace("%", "", regex=False).str.replace("٪", "", regex=False)
                df[c] = to_numeric_ignore(s2)
            except Exception:
                pass
    return df
